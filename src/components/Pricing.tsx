...
const Pricing: React.FC = () => {
  ...
  // const handleCheckout = useRazorpayCheckout();
      ...
      <div className="py-24 pb-0 px-8 max-w-5xl mx-auto">
        <div className="flex flex-col text-center w-full mb-20">
          ...
          <p className="text-sm md:text-base flex justify-center items-center gap-2">
            <span>
              <span className="text-lime-500">{highlighted_text}</span>{" "}
              {normalText}
            </span>
          </p>
        </div>
          ...
          {plans.map((plan: Plan, index) => (
            <div
              ...
              className={`relative w-full rounded-xl border transition-all duration-300
                ${
                  plan.popular
                    ? "border-2 border-primary-color bg-cards-bg/55 hover:shadow-lg hover:shadow-primary-color/20"
                    : "border-elements-secondary bg-cards-bg/20"
                }`}
            >
              {plan.badge && (
                <div className="absolute cursor-default px-3 rounded-xl top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 bg-primary-color hover:bg-primary-color/90 transition-colors duration-200">
              ...
              {plan.badge && (
                <div className="absolute -inset-[1px] rounded-[8px] z-10" />
              )}

              <div className="relative flex flex-col gap-5 lg:gap-8 z-10 p-8 rounded-lg h-full">
                  ...
                  <ButtonPrimary
                    ...
                    onClick={() => {
                      if (plan.btnLink) {
                        ...
                        // handleCheckout({ amount: prices.discountedPrice });
