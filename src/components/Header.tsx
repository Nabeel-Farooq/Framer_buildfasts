...
const Header: React.FC = () => {
  ...
  return (
    <header
      className="max-w-7xl bg-navbar-bg mx-auto px-8 py-5 flex items-center justify-between md:py-10"
      id="Navbar"
    >
      <div className="flex flex-row items-center align-middle gap-1 md:w-40">
        ...
        <motion.img
          ...
          alt={`${brand} Logo`}
          fetchPriority="high"
          className="w-8 h-8 text-logo-text-color"
        ...
        */}
        ...
        <h1 className="text-logo-text-color text-xl font-semibold">
          {brand}
        </h1>
      </div>
      ...
      <nav className="flex items-center pl-12 md:pl-20 gap-4 md:gap-4">
        <div className="hidden md:flex items-center space-x-4">
          {links.map((link, index) => (
            <a
              key={`${link.text}-${index}`}
              href={link.href}
              className="text-foreground-hsl/95 font-brico hover:text-foreground-hsl/75 hover:underline px-3 py-2 transition-colors duration-200"
            >
              {link.text}
            </a>
          ))}
        ...
        <div className="hidden md:flex items-center md:flex-row md:gap-3">
          <a
            ref={themeLinkRef}
            href="#"
            className="cursor-pointer bg-transparent border font-brico text-nav-normal-btn-text border-navbar-border-color shadow-sm hover:border-nav-links-color px-4 py-2 rounded-lg flex flex-row items-center gap-2 transition-all duration-200 group"
            onClick={(e) => {
              e.preventDefault();
              togglePopup();
            }}
          ...
          <a
            href={cta.href}
            className="bg-nav-cta-btn-bg border border-nav-cta-btn-border font-brico text-foreground-opposite px-4 py-2 rounded-lg flex flex-row items-center gap-1 transition-all duration-200 group"
            onClick={(e) => {
              e.preventDefault();
        ...
        <div className="md:hidden flex items-center">
          <button
            ...
            className="p-3 border border-gray-500 rounded-lg focus:outline-none transition-transform duration-300"
          >
